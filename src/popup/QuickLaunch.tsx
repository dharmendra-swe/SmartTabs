    ...
    return (
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3 hide-scrollbar">
            {workspaces.map(workspace => (
                <>
                    <Card key={workspace.id} className="p-3 flex items-center justify-between group cursor-pointer hover:border-[var(--border-focus)] transition-colors">
                        ...
                        {isLaunching === workspace.id ? (
                            ...
                            <div title="Launch Workspace" onClick={() => isLaunching !== workspace.id && onLaunch(workspace.id)}>
                                <Rocket className="w-4 h-4 mr-2 opacity-0 group-hover:opacity-100 transition-opacity hover:scale-120 hover:text-[var(--color-primary)] hover:animate-fade-in
                                 transition-transform text-[var(--text-muted)] fill-current" />
                            </div>
                        )}
                    </Card>
                </>
